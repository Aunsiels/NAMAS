local MixtureTablePerso, parent = torch.class('nn.MixtureTablePerso', 'nn.Module')

function MixtureTablePerso:__init(dim)
   parent.__init(self)
   self.dim = dim
...
function MixtureTablePerso:updateOutput(input) 
...
function MixtureTablePerso:updateGradInput(input, gradOutput)
   local gaterInput, expertInputs = table.unpack(input)
...
function MixtureTablePerso:type(type, tensorCache)
   self._gaterView = nil
   self._expert = nil
...
function MixtureTablePerso:clearState()
   nn.utils.clear(self, {
     '_gaterView',
     '_expert',
     '_expertView',
