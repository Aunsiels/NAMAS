...
#find $AGIGA/test | parallel --gnu --progress -j $THREADS python2.7 $SCRIPTS/process_dailymail.py \{\} $WORK
#find $AGIGA/training | parallel --gnu --progress -j $THREADS python2.7 $SCRIPTS/process_dailymail.py \{\} $WORK
#find $AGIGA/validation | parallel --gnu --progress -j $THREADS python2.7 $SCRIPTS/process_dailymail.py \{\} $WORK


echo "Step 2: Compile the data into train/dev/test."
#cd $WORK
#find $WORK/training | xargs cat > $WORK/train.data.txt
#find $WORK/validation | xargs cat > $WORK/valid.data.txt
#find $WORK/test  | xargs cat > $WORK/test.data.txt
#cat $SPLITS/test3.splits  | xargs cat > test.data.txt
